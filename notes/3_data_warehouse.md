...
* ***OLTP***: Online Transaction Processing.
    ...
    OLTP systems focus on managing real-time, day-to-day transaction data. These databases are typically normalized to ensure consistency and speed for insert, update, and delete operations.
    
    e.g. E-commerce Transaction Systems (Amazon order processing)

* ***OLAP***: Online Analytical Processing.

    OLAP systems are designed for complex queries over large volumes of historical or aggregated data. They support business intelligence, advanced analytics, and reporting, often using denormalized schemas (like star or snowflake schemas) to optimize read performance.

    e.g. Corporate Data Warehouses implementations using Snowflake, Amazon Redshift, Google BigQuery

    e.g. Business Intelligence (BI) Platforms (Example: Power BI)
...
- **Normalized database**: split data into logical, smaller tables (with relationships defined by foreign keys), so each piece of information appears only once, minimizing anomalies and redundant data storage.
- **Denormalized database** intentionally introduces some redundancy for performance or convenience in data retrieval. Instead of breaking everything into small tables, certain tables may be combined, or extra columns might be added (duplicating data) to reduce the number of joins and speed up queries.
...
Data warehouses provides all these possibilities to directly access raw data, summary data, or data marts.
...
* ***Ingestion time*** (_PARTITIONTIME): tables are partitioned based on the timestamp when BigQuery ingests the data.
* ***Integer range***: tables are partitioned based on an integer column.

For Time-unit and Ingestion time columns, the partition may be daily (the default option), hourly, monthly or yearly. (when using an hourly partition, might need an expire partitioning strategy, to avoid exceeding the 4000 limit)

>Note: BigQuery limits the number of partitions to 4000 per table. If you need more partitions, consider [clustering](#clustering) as well.

  e.g. If you partition by date (daily), you could theoretically have one partition for each date. If you exceed 4000 distinct dates (i.e., over 4000 days), you’ll hit the partition limit.
...
This is useful to check if there are data imbalances and/or biases in your partitions. (if some of the partitions are getting more rows and data, whereas other partitions are getting less data)
...
- Clustering can be done based on one or multiple columns up to 4 (your `CLUSTER BY` clause can list at most four columns. It does not mean you are limited to only four “clusters” in the sense of data segments or distinct values)
- The ***order*** of the columns in which the clustering is specified is important in order to determine the column priority (to determine the sort order of the data).
...
- A top-level column is one you see immediately under “Schema” when you look at your table’s definition (not nested inside another field).
  ```sql
  CREATE TABLE `mydataset.customers`
  (
    customer_id STRING NOT NULL,       -- top-level, non-repeated
    signup_date DATE,                  -- top-level, non-repeated
    country STRING,                    -- top-level, non-repeated
    preferences RECORD (               -- nested, but not repeated
  ...
  ```
- non-repeated means the column can store exactly one value per row (though it could be NULL if nullable), but not an array.
It **does not** mean the values must be unique across rows. It just means each row holds at most one value for that column, rather than a list of multiple values.
  ```sql
  CREATE TABLE `mydataset.UserInterests`
  (
    user_id INT64,
    interests ARRAY<STRING>  -- This is a repeated field
  )
  ```
  e.g. A single row might look like: interests = ["Hiking", "Photography", "Reading"] - Here, one row can contain multiple values in interests

  ```sql
  CREATE TABLE `mydataset.UserProfile`
  (
    user_id INT64,
    name STRING,       -- non-repeated (one value per row)
    email STRING       -- non-repeated (one value per row)
  )
  ```

For clustering columns, the following datatypes are supported:
* `DATE`
* `BOOL`
* `GEOGRAPHY`
* `INT64`
...
vs.
```sql
PARTITION BY DATE(tpep_pickup_datetime), VendorID
```
-> 1 partition per unique (date, vendor) combination

e.g. 2023-01-01 / Vendor A → Partition #1; 2023-01-01 / Vendor B → Partition #2; 2023-01-02 / Vendor A → Partition #3; 2023-01-02 / Vendor B → Partition #4 ...
-- this doesn't make sense, and can easily exceed the 4000-partition limit
...
As mentioned before, you may combine both partitioning and clustering in a table, but there are important differences between both techniques that you need to be aware of in order to decide what to use for your specific scenarior:
...
- your mutation operations modify the majority of partitions in the table frequently (for example, writing to the table every few minutes and writing to most of the partitions each time rather than just a handful).

BigQuery has _automatic reclustering_: 

when new data is written to a clustered table,
- it can be written to blocks that contain key ranges that overlap with the key ranges in previously written blocks (but does not overlap with your clustering strategy, it does not reorder the entire table)
- These overlapping key ranges weaken the sort property of the table. 
...
* Cost reduction
  * Avoid `SELECT *` (you should specify the name of the columns selected). Reducing the amount of columns to display will drastically reduce the amount of processed data and lower costs.
  ...
  * Use [streaming inserts](https://cloud.google.com/bigquery/streaming-data-into-bigquery) with caution. They can easily increase cost.
    - Streaming inserts (`INSERT` using BigQuery’s streaming API) allow real-time data ingestion, but they incur additional costs. - Use streaming only when real-time data is required, such as financial tickers, fraud detection, or live analytics dashboards.
    - Instead of streaming, batch loading (using `LOAD DATA` or `bq load`) is cheaper and more efficient. - Use batch loads (bq load, Dataflow, or Airflow DAGs) for scheduled data ingestion (e.g., every 15 minutes or hourly).
  * [Materialize query results](https://cloud.google.com/bigquery/docs/materialized-views-intro) in different stages.
    - Instead of running one giant query, break it into smaller stages and save intermediate results into a temporary or permanent table.
    - BigQuery charges for query execution based on the amount of data scanned. A single, complex query might recalculate the same subqueries multiple times, increasing cost.

      e.g. instead of:
      ```sql
      SELECT user_id, COUNT(*) 
      FROM ( 
        SELECT user_id, product_id, category FROM transactions WHERE event_type = 'purchase'
      )
      GROUP BY user_id;
      ```
      do:
      ```sql
      CREATE OR REPLACE TABLE mydataset.temp_purchases AS
      SELECT user_id, product_id, category FROM transactions WHERE event_type = 'purchase';

      SELECT user_id, COUNT(*) FROM mydataset.temp_purchases GROUP BY user_id;
      ```
...
* Query performance
  ...
  * [Denormalize data](https://cloud.google.com/blog/topics/developers-practitioners/bigquery-explained-working-joins-nested-repeated-data).
    - to reduce the number of joins: Joins in BigQuery are costly since data is stored in a distributed columnar format rather than row-based. Fewer joins = faster queries
    - example:
    ![normalized1](images/03_09.png)
    ![normalized2](images/03_10.png)
    ![denormalized](images/03_11.png)

  * Use [nested or repeated columns](https://cloud.google.com/blog/topics/developers-practitioners/bigquery-explained-working-joins-nested-repeated-data). (that will help you in denormalizing your data)
  ...
  * Do not treat `WITH` clauses as [prepared statements](https://www.wikiwand.com/en/Prepared_statement).
    - The `WITH` clause (common table expressions, or CTEs) does not cache results between different references. If you use `WITH` multiple times within the same query, BigQuery recomputes it every time.
    - instead of:
      ```sql
      WITH purchases AS (
        SELECT * FROM transactions WHERE event_type = 'purchase'
      )
      SELECT user_id, COUNT(*) FROM purchases GROUP BY user_id
      UNION ALL
      SELECT user_id, COUNT(*) FROM purchases GROUP BY user_id;
      ```
      Use a temporary table or materialized result:
      ```sql
      CREATE OR REPLACE TABLE mydataset.purchases AS
      SELECT * FROM transactions WHERE event_type = 'purchase';

      SELECT user_id, COUNT(*) FROM mydataset.purchases GROUP BY user_id
      UNION ALL
      SELECT user_id, COUNT(*) FROM mydataset.purchases GROUP BY user_id;
      ```

  * Avoid [oversharding tables](https://cloud.google.com/bigquery/docs/partitioned-tables#dt_partition_shard).
    - Do not create too many small tables for different categories of data (sharding). Instead, use partitioning or clustering.
    - Instead of creating separate tables like: transactions_20240101, transactions_20240102...
  ...
  * Use [approximate aggregation functions](https://cloud.google.com/bigquery/docs/reference/standard-sql/approximate_aggregate_functions) rather than complete ones such as [HyperLogLog++](https://cloud.google.com/bigquery/docs/reference/standard-sql/hll_functions).
    ...
    ```
    Accuracy trade-off: `APPROX_COUNT_DISTINCT` is 99% accurate but much faster and cheaper.

  * Order statements should be the last part of the query, for query operations to maximize performance
  * [Optimize join patterns](https://cloud.google.com/bigquery/docs/best-practices-performance-compute#optimize_your_join_patterns):
    * As a best practice, Place the table with the _largest_ number of rows first, followed by the table with the _fewest_ rows, and then place the remaining tables by decreasing size.
    * This is due to how BigQuery works internally: the first table will be distributed evenly and the second table will be broadcasted to all the nodes. Check the [Internals section](#internals) for more details.
      - The first table in a join (the largest one) gets shuffled across multiple processing nodes. Each node gets **a portion** of the data, meaning the table is **split into chunks** and distributed evenly. This allows BigQuery to parallelize the processing efficiently when performing the join.
      - The second table (smaller one) is **copied to every processing node** rather than being split. This means **every node gets the full small table to perform the join**.
...
* ***Jupiter***: the network that connects Dremel (for computes) and Colossus (for storage)
...
BQ root server receives a query

-> it divides it into smaller sub modules (R1i UNION ALL ... R1n)

-> it divides the query further into R1 to R1n

-> the mixers generally get the modified query, which is then further divided into other subset of queries given to leaf nodes

-> the leaf nodes are the one which actually query or actually talk to the colossus database, fetches the data, executes appropriate operations on top of that, and returns the data back to mixers,which then return to root servers and the data is aggregated and returned as a result

(BQ is smart enough to divide the query into smaller chunks and propagate them to the leaf nodes, so that's why BQ is so fast)
### Reference
...
* The `OPTIONS()` clause contains all of the necessary arguments to create our model:
  ...
  * `input_label_cols=['tip_amount']` lets BQ know that our target feature is `tip_amount` (what I want to predict). For linear regression models, target features must be real numbers.
...
#### [Reference](https://cloud.google.com/bigquery-ml/docs/reference)
...
1. Based on [the original DAG file we used in lesson 2](../02-workflow-orchestration/airflow/dags/data_ingestion_gcs_dag_new.py), we will copy the necessary imports and additional code.
...
1. Create the `gcs_2_gcs` task.
    ...
    * We want to ***move*** multiple files. We will need to define the source bucket and objects as well as the destination bucket and objects.
        ...
        * The destination object filename can be thought of as the prefix to be added to the source object filename.
        ![wildcard](images/03_19.png)
    * Moving or copying files within Google Cloud Storage is essential for efficient data workflows, cost management, and system reliability. 
...
The finalized `gcs_2_bq_dag.py` can be downloaded [from this link](../03-data-warehouse/airflow/dags/gcs_2_bq_dag.py).

![dag](images/03_20.png)
![bq](images/03_21.png)
![partitioned](images/03_22.png)
