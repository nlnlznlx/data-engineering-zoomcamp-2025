...
def main(params):
    ...
    file_name = url.rsplit('/', 1)[-1].strip() # rsplit('/', 1) means starting split from the last slash and split it into two parts
    ...
    if '.csv' in file_name:
        df = pd.read_csv(file_name, nrows=10) # When creating a table in the database (df.head(0).to_sql(...)), the script needs to know the column names and their data types
        df_iter = pd.read_csv(file_name, iterator=True, chunksize=100000)
    elif '.parquet' in file_name:
        file = pq.ParquetFile(file_name)
        
        df = next(file.iter_batches(batch_size=10)).to_pandas()
        # iter_batches() method returns an iterator, which is a sequence of data batches
        # next() function fetches the first batch from this iterator
        # Batches returned by iter_batches() are in pyarrow format, which is not directly compatible with pandas. The .to_pandas() method converts the pyarrow batch into a pandas DataFrame
